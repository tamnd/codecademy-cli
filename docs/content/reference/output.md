...
describes what users get. Pick a format with `-o`, or let codecademy choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
codecademy <command> -o table   # aligned columns for reading
codecademy <command> -o jsonl   # one JSON object per line, for piping
codecademy <command> -o json    # a single JSON array
codecademy <command> -o csv     # spreadsheet friendly
codecademy <command> -o tsv     # tab-separated
codecademy <command> -o url     # just the URL column
codecademy <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
codecademy <command> --fields id,title,url
```
...
```bash
codecademy <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
codecademy <command>            # a table, because this is a terminal
codecademy <command> | wc -l    # JSONL, because this is a pipe
```
