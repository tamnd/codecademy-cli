// Package cli builds the codecademy command tree on top of the codecademy library.
package cli
...
type App struct {
	client   *codecademy.Client
	cfg      codecademy.Config
	output   string
	fields   []string
	noHeader bool
	template string
...
func Root() *cobra.Command {
	app := &App{cfg: codecademy.DefaultConfig()}

	root := &cobra.Command{
		Use:           "codecademy",
		Short:         "Browse and search the Codecademy course catalog",
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = codecademy.NewClient(a.cfg)
	return nil
}
